...
procedure fldbig;//рисует шаблон, проверяет на вмещаемость и корректирует поле
begin
  var xxx:=x;
  var yyy:=y;
  drawrec(x,y,frm[q]);
  if x>xfield then  //искусственное увеличение поля по x
        begin
          while x>xfield do
          begin
            pen.Width:= 1;
        ...
        if y>yfield then  //искусственное увеличение поля по y
        begin
          while y>yfield do
          begin
            pen.Width:= 1;
            GraphABC.SetPenColor(GraphABC.RGB(179, 179, 179));
            for var xfx:=1 to xfield do
            begin
              p.Line(xfx*kl,yfield*kl,xfx*kl,(yfield+1)*kl);
            end;
            p.Line(0,(yfield+1)*kl,xfield*kl,(yfield+1)*kl);
            yfield:=yfield+1;
            setPenColor(clblack);
            drawrec(xxx,yyy,frm[q]);
          end;
