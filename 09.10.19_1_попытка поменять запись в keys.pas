...
begin
    ...
    if length(strk)>4 then
    begin
      if strk[length(strk)]=strk[length(strk)-1] then
      begin
        setlength(strk,length(strk)-1);
      end;
    end;
    close(keyt);
    append(keyt);}
...
begin
  ...
  writeln('Do you want to work with parameters from file?');
  readln(ans);
  if ans='no' then
  begin
  writeln('How many tasks do you want to create?');
  readln(task);
  writeln('Which scale do you want?');
  readln(kl);
  writeln('How many forms do you want to use?');
  readln(rfrm);
  writeln('How long way do you want?');
  readln(rwayl);
  writeln('From what number do you want to start?');
  readln(ak);
  end
  else parameter;
  assign(keyt,'keys.txt');
